...
import SectionCoursePlayer from "@/components/course/SectionCoursePlayer";
import {
...
const CourseSectionPage = async ({ params }: CourseSectionPageProps) => {
  ...
  const activeSection = getSectionById(sections, sectionId);
  if (!activeSection) {
    notFound();
  }
        ...
        <section className="rounded-card border border-odyssey-gray-light/60 bg-white p-8 shadow-[0_20px_50px_rgba(17,17,17,0.08)]">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            <div>
              ...
              <p className="mt-4 text-sm text-odyssey-gray">{course.meta.summary}</p>
              <div className="mt-6 flex flex-wrap gap-3 text-xs uppercase tracking-[0.3em] text-odyssey-gray">
                ...
                <span className="rounded-pill border border-odyssey-gray-light bg-odyssey-gray-light/40 px-3 py-1">
                  Section {sections.findIndex((section) => section.id === activeSection.id) + 1}{" "}
                  of {sections.length}
                </span>
              </div>
            </div>
